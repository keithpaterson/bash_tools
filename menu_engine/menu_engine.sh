...
#
# This script, when sourced, will automatically try to load menu_engine_common.sh
...
if [ -z "${ME_COMMON}" ]; then
  # This only works if menu_engine_common.sh is in the same place as this file.
  _local_dir=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
  source ${_local_dir}/menu_engine_common.sh
fi
